...
var Menu = function (doSize) {
  ...
  tabs.addEventListener('click', function (e) {
    ...
    for (var i = 0; i < items.childNodes.length; i++) {
      if (items.childNodes[i].style.opacity == '1') {
        items.childNodes[i].style.removeProperty('opacity');
      }
    }
    items.childNodes[visible[0]].style.opacity = '1';
    if (visible.length == 2) {
      items.childNodes[visible[1]].style.opacity = '1';
    }
    // Move pages
    for (var i = 0; i < items.childNodes.length; i++) {
      if (i == index) {
    ...
    if (isDropdown) {
      if (target.getAttribute('selected') == 'true') {
        ...
        for (var i = 0; i < formats.length; i++) {
          if (formats[i].hasAttribute('selected')) {
            formats[i].removeAttribute('selected');
          }
        }
    ...
    initialize: function (_vInfo) {
      ...
      function map (str) {
        ...
        case 'hd1080':
          return '1080p';
        default:
          return (str || '').toLowerCase().replace(/./, function ($1) {
      ...
      vInfo.formats = vInfo.formats.filter(function (elem) {
        if (self.options.showFLV && elem.container === 'flv') {
          return true;
        }
        if (self.options.showWEBM && elem.container === 'webm') {
          return true;
        }
        if (self.options.showMP4 && (elem.container === 'mp4' || elem.container === 'm4a')) {
          return true;
        }
        if (self.options.show3GP && elem.container === '3gp') {
          return true;
        }
